...
for q in qList:
    ...
    for count in range(numTrials):
        ...
        neighborsOfFire1 = getValidOpenVal1(firstCellOnFire1[0], firstCellOnFire1[1], size1)
...
for q in qList:
    ...
    for count in range(numTrials):
        ...
        neighborsOfFire2 = getValidOpenVal1(firstCellOnFire2[0], firstCellOnFire2[1], size2)
...
for q in qList:
    ...
    for count in range(numTrials):
        ...
        neighborsOfFire3 = getValidOpenVal1(firstCellOnFire3[0], firstCellOnFire3[1], size3)
        ...
        while arr3[bot3[0]][bot3[1]][0] != 2 and arr3[button3[0]][button3[1]][0] != 2:
            ...
            validOpen = getValidOpenVal1(bot3[0], bot3[1], size3, arr3)
            listToTraverse = []

            if validOpen:
                listToTraverse = validOpen
            else:
                listToTraverse = getValidOpenVal3(bot3[0], bot3[1], size3, arr3)
...
for q in qList:
    ...
    for count in range(numTrials):
        ...
        while arr4[bot4[0]][bot4[1]][0] != 2 and arr4[button4[0]][button4[1]][0] != 2:
            ...
            # check if button is surrounded by only closed and on fire cells
            buttonNeighbors = getValidOpen(button4[0], button4[1], size4, arr4, firstCellOnFire4, visited)
            noOpenAroundButton = True
            for neighbor in buttonNeighbors:
                if arr4[neighbor[0]][neighbor[1]][0] == 1:
                    noOpenAroundButton = False
                    break
            if noOpenAroundButton:
                break
            ...
            if validOpen:
                ...
                listToTraverse = getValidOpenVal3(bot4[0], bot4[1], size4, arr4, visited)

            if listToTraverse:
                distsOpenToButton = []
                for neighbor in listToTraverse:
                    neighborToButton = findDistanceBetweenBot4(neighbor[0], neighbor[1], button4[0], button4[1], size4, arr4)
                    if neighborToButton == -1:  # no path found, so set this val to inf
                        distsOpenToButton.append(float('inf'))
                    else:
                        distsOpenToButton.append(neighborToButton)
                ...
                if smallestIndex != -1:
                    bot4 = listToTraverse[smallestIndex]
            else:
                break
...
for item in winsHashtableBot4:
    index = list(winsHashtableBot4.keys()).index(item)
    yVals3.append(winsHashtableBot4[item] / numTrialsPerQVal[index])
...
plt.plot(qList, yVals4, label='Bot 3', color='c')
halfConfidence = widthOfInterval / 2
plt.fill_between(qList, [y - halfConfidence for y in yVals1], [y + halfConfidence for y in yVals1], color='m',
                 alpha=0.5, label='Confidence Interval Bot 1')
plt.fill_between(qList, [y - halfConfidence for y in yVals2], [y + halfConfidence for y in yVals2], color='b',
                 alpha=0.5, label='Confidence Interval Bot 2')
plt.fill_between(qList, [y - halfConfidence for y in yVals3], [y + halfConfidence for y in yVals3], color='g',
                 alpha=0.5, label='Confidence Interval Bot 3')
