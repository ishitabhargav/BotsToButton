...
def newSpreadFireBot12(arr, neighborsOfFire, size, q, fireCells) -> ndarray[Any, dtype[Any]]:
    ...
    for neighbor in neighborsOfFire:
        if arr[neighbor[0]][neighbor[1]][0] == 1:
            ...
            if rand < prob:  # the cell catches on fire
                tempShip[neighbor[0]][neighbor[1]][0] = 2
...
def newSpreadFireBot3(arr, neighborsOfFire, size, q, fireCells) -> ndarray[Any, dtype[Any]]:
    tempShip = np.copy(arr)
    newFireNeighbors = []
    for neighbor in neighborsOfFire:
        if arr[neighbor[0]][neighbor[1]][0] == 3:
            ...
            if rand < prob:  # the cell catches on fire
                ...
                neighborsOfNewFireCell = getValidNeighbors(neighbor[0], neighbor[1], size)
                for newNeighbor in neighborsOfNewFireCell:
                    # check to make sure it's not a current neighbor of fire, it's an open cell, and it hasn't been marked
                    # before as a new fire neighbor (we don't want to add the same cell twice to newFireNeighbors if it has
                    # multiple neighbors catching on fire
                    if newNeighbor not in neighborsOfFire and arr[newNeighbor[0]][newNeighbor[1]][0] == 1 and \
                            tempShip[newNeighbor[0]][newNeighbor[1]][0] != 3:
                        tempShip[newNeighbor[0]][newNeighbor[1]][0] = 3
                        newFireNeighbors.append(newNeighbor)
    for neighbor in newFireNeighbors:
        if neighbor not in neighborsOfFire:
...
def newSpreadFireBot4(arr, neighborsOfFire, size, q, fireCells, neighborsOfNeighborsFire, cost1F, cost2F) -> ndarray[
    Any, dtype[Any]]:
    tempShip = np.copy(arr)
    newFireNeighbors = []
    newNeighborsOfFireNeighbors = []
    remove_neighbor_of_fire = []
    for neighbor in neighborsOfFire:
        if arr[neighbor[0]][neighbor[1]][0] == cost1F:  # direct neighbor of a fire cell
            ...
            if rand < prob:  # the cell catches on fire
                tempShip[neighbor[0]][neighbor[1]][0] = 2
                # neighborsOfFire.remove(neighbor)
                remove_neighbor_of_fire.append(neighbor)
                fireCells.append(neighbor)
                neighborsOfNewFireCell = getValidNeighbors(neighbor[0], neighbor[1], size)
                for newNeighbor in neighborsOfNewFireCell:
                    # if newNeighbor not in neighborsOfFire and arr[newNeighbor[0]][newNeighbor[1]][0] == 1:
                    if arr[newNeighbor[0]][newNeighbor[1]][0] == cost2F or arr[newNeighbor[0]][newNeighbor[1]][0] == 1:
                        if arr[newNeighbor[0]][newNeighbor[1]][0] == cost2F and newNeighbor in neighborsOfNeighborsFire:
                            neighborsOfNeighborsFire.remove(newNeighbor)
                            # print('this is ok')
                        tempShip[newNeighbor[0]][newNeighbor[1]][0] = cost1F  # now cell is a direct neighbor of fire
                        newFireNeighbors.append(newNeighbor)
                        dist2FromFire = getValidOpenVal145(newNeighbor[0], newNeighbor[1], size, arr, cost1F,
                                                           cost2F)
                        for dist2 in dist2FromFire:
                            if tempShip[dist2[0]][dist2[1]][0] == 1: # it's an open cell
                                tempShip[dist2[0]][dist2[1]][0] = cost2F  # now cell is a neighbor of a neighbor on fire
    ...
    # remove cells that used to be neighbors of fire cells but are now on fire
    for element in remove_neighbor_of_fire:
        neighborsOfFire.remove(element)
    return tempShip
    ...
    for count in range(numTrials):
        ...
        while arr1[bot1[0]][bot1[1]][0] != 2 and arr1[button1[0]][button1[1]][0] != 2:
            ...
            if bot1[0] == button1[0] and bot1[1] == button1[1]:  # bot == button: #
                ...
                winsHashtableBot1[q] = winsHashtableBot1[q] + 1
                break
            arr1 = newSpreadFireBot12(arr1, neighborsOfFire1, size1, q, fireCells1)
    ...
    for count in range(numTrials):
        ...
        while arr2[bot2[0]][bot2[1]][0] != 2 and arr2[button2[0]][button2[1]][0] != 2:
            ...
            if bot2 == button2:
                ...
                winsHashtableBot2[q] = winsHashtableBot2[q] + 1
                break
            arr2 = newSpreadFireBot12(arr2, neighborsOfFire2, size2, q, fireCells2)
...
for q in qList:
    ...
    for count in range(numTrials):
        ...
        while arr3[bot3[0]][bot3[1]][0] != 2 and arr3[button3[0]][button3[1]][0] != 2:
            ...
            noOpenAroundButton = True
            for neighbor in buttonNeighbors:
                if arr3[neighbor[0]][neighbor[1]][0] == 1 or arr3[neighbor[0]][neighbor[1]][0] == 3:  # add if it's 3
                    noOpenAroundButton = False
                    break
            if noOpenAroundButton:
                break
            ...
            if distWithout3 != -1:
                ...
                for neighbor in listToTraverse:
                    neighborToButton = findDistanceBetweenBot3(neighbor[0], neighbor[1], button3[0], button3[1], size3,
                                                               arr3)
                    if neighborToButton == -1:  # no path found, so set this val to inf
                        distsOpenToButton.append(float('inf'))
                    else:
            ...
            if bot3 == button3:
                ...
                winsHashtableBot3[q] = winsHashtableBot3[q] + 1
                break
            arr3 = newSpreadFireBot3(arr3, neighborsOfFire3, size3, q, fireCells3)'''
...
for q in qList:
    ...
    for count in range(numTrials):
        ...
        neighborsOfFire4 = getValidOpenVal145(firstCellOnFire4[0], firstCellOnFire4[1], size4, arr4, cost1F,
                                              cost2F)
        neighborsOfNeighborsFire = []
        for neighbor in neighborsOfFire4:
            arr4[neighbor[0]][neighbor[1]][0] = cost1F  # mark as a neighbor of a fire cell
            listNeighbors = getValidOpenVal145(neighbor[0], neighbor[1], size4, arr4, cost1F, cost2F)
            for neighborOfNeighbor in listNeighbors:
                if neighborOfNeighbor not in neighborsOfNeighborsFire and neighborOfNeighbor not in neighborsOfFire4 and neighborOfNeighbor != firstCellOnFire4:
                    arr4[neighborOfNeighbor[0]][neighborOfNeighbor[1]][0] = cost2F # mark this neighbor as a neighbor of a fire cell
        ...
        dist1FromButton = getValidOpenVal145(button4[0], button4[1], size4, arr4, cost1F, cost2F)
        ...
        for dist1 in dist1FromButton:
            ...
            dist2FromButton = getValidOpenVal145(dist1[0], dist1[1], size4, arr4, cost1F, cost2F)
            for dist2 in dist2FromButton:
                if dist2 not in markedCells:
                    arr4[dist2[0]][dist2[1]][1] = benefitButton2
                    markedCells.append(dist2)
                    dist3FromButton = getValidOpenVal145(dist2[0], dist2[1], size4, arr4, cost1F, cost2F)
        ...
        while arr4[bot4[0]][bot4[1]][0] != 2 and arr4[button4[0]][button4[1]][0] != 2:
            ...
            if noOpenAroundButton:
                break
            prevCell = bot4
            validOpen = getValidOpenVal145(bot4[0], bot4[1], size4, arr4, cost1F, cost2F)

            if validOpen:
                distsOpenToButton = []
                for neighbor in validOpen:
                    neighborToButton = findDistanceBetweenBot4(neighbor[0], neighbor[1], button4[0], button4[1], size4,
                                                               arr4, q, cost1F, cost2F, benefitButton1,
                                                               benefitButton2, benefitButton3)
            ...
            if bot4 == button4:  # bot2[0] == button2[0] and bot2[1] == button2[1]:
                ...
                winsHashtableBot4[q] = winsHashtableBot4[q] + 1
                break
            arr4 = newSpreadFireBot4(arr4, neighborsOfFire4, size4, q, fireCells4, neighborsOfNeighborsFire, cost1F,
                                     cost2F)
